...
class ZteReadout(SequenceModule):
    ...
    g_read : list of list of GradEvent
        ...
        next view. The last entry slews to zero instead, ending the shell.
    adc : AdcEvent
    ...
    shot_rotations : numpy.ndarray
        ``(n_shots, 3, 3)``, the rotation about ``z`` that maps the shell onto
        each shot's spoke directions. Published only when the module generated
        the shell, not when ``directions`` is supplied.
    directions : numpy.ndarray
        ``(n_views, 3)``, the spoke directions of one shell, in play order.
    step_rad : float
        The widest angle between consecutive views. Every turn between views
        is allotted the duration of this step, so a shell with equal steps has
        no unused turn time.
    tr : float
    ...
    directions : array_like, default=None
        ...
        them. When supplied, the four generator arguments below are ignored;
        by default the shell is generated by ``calc_projection_shell``. An
        ordering whose steps vary is accepted, but every turn is allotted the
        duration of the widest step, so the repetition is timed for the
        largest step throughout.
    ...
    ValueError
        ...
        dead-time gap covers the whole spoke, two consecutive views coincide, or the
