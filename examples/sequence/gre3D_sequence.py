"""RF-spoiled 3D Cartesian gradient echo, slab-selective."""
...
class Gre3DApp(sequences.SequenceApp):
    """RF-spoiled, slab-selective 3D Cartesian gradient echo.

    One line per repetition, phase-encoded along y and z over a CAIPIRINHA
    lattice. The fully sampled calibration rectangle leads the scan as segment
    0 with ``IMA`` set; the other pairs are segment 1. Under a wave corkscrew
    the rectangle is first acquired again wave-free and marked ``REF``, and the
    imaging pairs carry no ``IMA``.

    Examples
    --------
    >>> from pypulseqpp import sequences
    >>> seq = sequences.gre3D_sequence(n_x=32, n_y=16, n_z=4, n_acs=0, n_acs_z=0, n_dummy=0)
    >>> seq.check_timing()[0]
    True
    ...
    def init_sequence(
        ...
        """Design the slab pulse, the readout and the ``(line, partition)`` order.
        ...
        n_dummy : int, optional
            Non-acquiring repetitions before the first pair, which let the
            RF-spoiled transient settle before the calibration rectangle.
    ...
    def kernel(
        ...
        ``flags`` are labels carried on the excitation with ``LIN`` and
        ``PAR``, and ``wave`` scales the corkscrew.
        """
        ...
        seq.add_block(ro.gx, *([] if view is None else [ro.adc]), *corkscrew)
        seq.add_block(
