...
def test_a_linear_train_steps_by_the_segmented_blip():
    order = calc_epi_order(5, acceleration=3, segments=2)
...
def test_segmenting_widens_the_blip_without_moving_the_lattice():
    ...
    whole = calc_epi_order(12, acceleration=2)[:, 0]
    parts = [
        residue * 2 + calc_epi_order(4, acceleration=2, segments=3)[:, 0]
...
def test_a_caipi_train_stays_inside_its_partition_cycle():
    order = calc_epi_order(32, scheme="caipi", partition_acceleration=5, caipi_shift=2)
...
def test_a_zigzag_turns_inside_its_segment():
    order = calc_epi_order(20, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_zigzag_never_steps_further_than_one_blip():
    order = calc_epi_order(40, scheme="zigzag", acceleration=6, extent=24)
...
def test_the_return_pass_samples_between_the_outward_one():
    ...
    order = calc_epi_order(7, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_zigzag_revisits_the_same_lines_at_later_echoes():
    order = calc_epi_order(21, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_fraction_that_loses_the_centre_is_refused(partial_fourier):
    with pytest.raises(ValueError, match=r"partial_fourier must be in \(0.5, 1\]"):
        calc_sampled_lines(32, 1, 0, partial_fourier=partial_fourier)
