...
def test_every_scheme_starts_at_the_shot_origin(kwargs):
    ...
    order = make_epi_shot_offsets(7, **kwargs)
...
def test_a_linear_train_steps_by_the_segmented_blip():
    order = make_epi_shot_offsets(5, acceleration=3, segments=2)
...
def test_segmenting_widens_the_blip_without_moving_the_lattice():
    ...
    whole = make_epi_shot_offsets(12, acceleration=2)[:, 0]
    parts = [
        residue * 2 + make_epi_shot_offsets(4, acceleration=2, segments=3)[:, 0]
...
def test_a_caipi_train_tiles_its_lattice_exactly(ny, nz, ry, rz, shift, segments):
    ...
    expected = make_caipirinha_mask((ny, nz), ry, rz, delta=shift)
...
def test_a_zigzag_turns_inside_its_segment():
    order = make_epi_shot_offsets(20, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_zigzag_never_steps_further_than_one_blip():
    order = make_epi_shot_offsets(40, scheme="zigzag", acceleration=6, extent=24)
...
def test_the_return_pass_samples_between_the_outward_one():
    ...
    order = make_epi_shot_offsets(7, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_zigzag_revisits_the_same_lines_at_later_echoes():
    order = make_epi_shot_offsets(21, scheme="zigzag", acceleration=4, extent=12)
...
def test_a_count_out_of_range_is_refused(kwargs):
    ...
    with pytest.raises(ValueError, match="positive"):
        make_epi_shot_offsets(etl, **kwargs)
...
def test_partial_fourier_keeps_the_far_side_of_k_space(partial_fourier):
    ...
    calibration, imaging = make_cartesian_axis_sampling(
        32, 1, 0, partial_fourier=partial_fourier
    )

    assert calibration == []
    assert imaging == list(range(32 - round(partial_fourier * 32), 32))
    assert 16 in imaging
...
def test_partial_fourier_truncates_the_calibration_block_with_everything_else():
    ...
    calibration, imaging = make_cartesian_axis_sampling(32, 2, 32, partial_fourier=0.75)

    assert min([*calibration, *imaging]) == 8
    assert calibration == sorted(calibration)


def test_partial_fourier_and_acceleration_compose():
    calibration, imaging = make_cartesian_axis_sampling(32, 2, 0, partial_fourier=0.75)

    assert calibration == []
    assert imaging == [line for line in range(8, 32) if (line - 16) % 2 == 0]
...
def test_a_fraction_that_loses_the_centre_is_refused(partial_fourier):
    with pytest.raises(ValueError, match=r"partial_fourier must be in \(0.5, 1\]"):
        make_cartesian_axis_sampling(32, 1, 0, partial_fourier=partial_fourier)
