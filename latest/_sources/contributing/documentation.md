...
An **API example** shows how an object is constructed and used. It belongs in the `Examples` section of a docstring or on the reference page for that object, it is a few lines long, and it needs no experiment, sweep or scientific result. Reference material that illustrates one object — including a representative configuration and a figure of its output — belongs here.
...
When examples fall into categories, the categories and the example cards belong on the gallery landing page: a reader browses a gallery visually, by scanning titles and thumbnails, rather than by expanding a navigation tree.

Global navigation should normally contain the gallery itself and nothing below it. Category pages generated by the gallery builder should not become primary navigation entries merely because they exist; individual examples remain reachable from the cards, from cross-references and from the gallery's own navigation.

Give each category a name that states what its examples have in common and one or two factual sentences of description. Do not add prose to fill the page.

Prefer the mechanisms the documentation builder supports — card directives, landing-page construction, navigation depth settings — over hiding generated pages with CSS.
