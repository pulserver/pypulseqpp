...
## Conventions common to the checks
...
## The checks and the data each requires
...
record, so they can always run. The last three require site or coil data that
no sequence file contains — a nerve model's coefficients, a magnet's forbidden
bands, a subject's VOP model and the transmit chain's calibration — and must be
...
preceding slew history. The stimulation check is therefore evaluated over the
whole sequence in one pass, starting from rest, rather than over a
