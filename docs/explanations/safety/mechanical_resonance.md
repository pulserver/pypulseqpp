...
mode's frequency drives that mode, which produces acoustic output far above what
the same amplitude produces elsewhere in the spectrum, and mechanical stress on
the assembly. Vendors publish the frequency ranges the coil must not be driven
in as **forbidden bands**, and
{func}`~pypulseqpp.safety.check_mech_resonance` compares the sequence's gradient
spectrum with them.

## Why a windowed spectrum
...
leaves it deposits far less energy than one that sits in it for a second. A
single transform of the whole sequence cannot express that distinction: it
...
## Why echo-planar and spiral readouts are the families at risk
...
1 kHz, and holds it there for the length of the train. A band either contains
that line or does not, which is why echo spacing is the parameter a vendor's
table constrains, and why some vendors publish their bands as forbidden
**echo-spacing** ranges rather than as frequencies.
...
few hundred hertz and spends most of each repetition at zero, so it rarely
reaches a band at all.
...
the report carries the worst window of every band whether or not it violates.
...
establishes is that the sequence does not hold sustained gradient amplitude
inside a range the table says the system must not be driven in.
...
* {doc}`../../generated/gallery/01-basics/02-analysis-and-checks` — the check
  and the spectrum it reads, on an echo-planar sequence.
